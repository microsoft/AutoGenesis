"""
Element utilities for Desktop Windows Skill.
Provides helper functions for element finding and snapshot extraction.
"""
...
async def extract_element_info_async(
    element: Any,
    max_root_depth: int = 10,
    web_depth: int = 0,
    limited_page_length: int = 15,
    in_limited_page: bool = False
) -> Dict[str, Any]:
    """
    Extracts information from a UI element and its children recursively.

    Args:
        element: The pywinauto UI element to extract information from
        max_root_depth: Maximum depth for root elements
        web_depth: Current depth in the web hierarchy
        limited_page_length: Maximum number of children to process
        in_limited_page: Boolean indicating if within a limited page

    Returns:
        A dictionary containing the extracted snapshot information
    """
    ...
    snapshot = {
        "snapshot_pywinauto": extract_element_info_from_pwa(
            element,
...
def extract_element_info_from_pwa(
    element: Any,
    max_root_depth: int = 10,
    web_depth: int = 0,
    limited_page_length: int = 15,
    in_limited_page: bool = False,
    runtime_ids: Optional[List] = None
) -> Optional[Dict[str, Any]]:
    """
    Extract element info from pywinauto element recursively.

    Args:
        element: The pywinauto element
        max_root_depth: Maximum depth for traversal
        web_depth: Current depth in web hierarchy
        limited_page_length: Max children per element
        in_limited_page: Whether in a limited page context
        runtime_ids: List of processed runtime IDs to avoid duplicates

    Returns:
        Dictionary with element info or None if size limit exceeded
    """
    global total_size, total_size_break
    ...
    # Check size limit
    if asizeof:
        info_size = asizeof.asizeof(info)
        if total_size + info_size > SNAPSHOT_MAX_SIZE:
            logger.warning(
                f"Snapshot size exceeded limit of {SNAPSHOT_MAX_SIZE} bytes, "
    ...
    # Check for web page root
    is_web_page_root = False
    if (element.element_info.automation_id == "RootWebArea" and
            element.element_info.control_type == "Document"):
    ...
    for child in element.children():
        ...
        idx_web_length += 1
        child_info = extract_element_info_from_pwa(
            child,
            max_root_depth=max_root_depth,
...
async def fill_snapshot(
    resp: Dict[str, Any],
    session_manager: Any,
    need_snapshot: int
) -> None:
    """
    Fill response with UI snapshot data.

    Args:
        resp: Response dictionary to fill
        session_manager: Windows session manager
        need_snapshot: 1 to include snapshot, 0 to skip
    """
    if need_snapshot == 1:
...
async def find_element_by_kwargs(
    ...
    """
    Find a UI element by various search criteria.
    ...
    Args:
        ...
        parent_control_idx: Parent element index
        parent_class_name: Parent class name
        timeout: Search timeout in seconds
        main_window_type: Type of main window
        search_type: 'normal' or 'fuzzy'

    Returns:
        Tuple of (element, exists, search_kwargs, parent_search_kwargs)
    """
    main_window = await session_manager.get_main_window(main_window_type=main_window_type)

    # Find parent element if specified
    search_parent = main_window
    parent_search_kwargs: Dict[str, Any] = {}
    ...
    # Fallback to fuzzy search if normal search fails
